...
def get_tasks():
    ...
    task = st.sidebar.radio("Choose one:",
          ("Load from local drive, <200MB", "Load from local drive, 200MB+", "Load from Google Storage", "Yahoo Finance") )
    return task
...
if task == "Load from local drive, <200MB":
    ...
    if dataset is not None: # moved this check to the beginning
        ...
        if addon:
            ...
            if llm == "PandasAI":
                ...
                connector = PandasConnector({"original_df": df}, field_descriptions=addon)
                sdf = SmartDataframe(connector, {"enable_cache": False}, config={"llm": llm, "conversational": False, "response_parser": OutputParser})
...
if task == "Load from Google Storage":
    bucket_name = st.text_input("Provide your bucket name from BigQuery.") # this line and subsequent lines in the if block need to be indented
...
if task == "Yahoo Finance":
    stock_symbol = st.text_input("Enter a stock symbol, e.g. MSFT.") # this line and subsequent lines in the if block need to be indented
